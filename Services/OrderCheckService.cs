...
namespace Food_maui.Services
{
    public class OrderCheckService
    {
        ...
        public async Task AcceptOrder(int orderId)
        {
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    var parameters = new { SalesOrderID = orderId };
                    var content = new StringContent(JsonSerializer.Serialize(parameters), Encoding.UTF8, "application/json");
                    var response = await client.PostAsync("http://99.89.32.196/api/Buisness/BusinessAcceptOrder", content);
    ...
    public class PendingOrderCheckService
    {
        ...
        private string salesOrderID = "2680";

        public PendingOrderCheckService()
        {
            StartOrderCheckTimer();
        }
        ...
        private void StartOrderCheckTimer()
        {
            _timer = new System.Timers.Timer(900000); // 1 minute interval
            _timer.Elapsed += async (sender, e) => await CheckNewPendingOrders();
            _timer.Start();
        }

        private async Task CheckNewPendingOrders()
        {
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    var content = new StringContent(JsonSerializer.Serialize(parameters), Encoding.UTF8, "application/json");
                    var response = await client.PostAsync("http://99.89.32.196/api/Buisness/PendingOrdersNotification", content);
                    ...
                    if (newOrder != "0")
                    {
                        ...
                        Console.WriteLine("New PendingOrder detected.");
                        _noOrderCount = 0;
                        ShowPopupNotification(notificationSubject, plainTextMsg);
                    }
                    else
        ...
        // public async Task AcceptPendingOrder(int orderId)
        // {
        //     try
        //     {
        //         Console.WriteLine("Accept Orders---------------------------------->");
